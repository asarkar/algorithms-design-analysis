...
/**
  ...
  * Rather, when there is a recursive call on a subarray of length m, you should simply add m - 1 to your running total
  * of comparisons. (This is because the pivot element is compared to each of the other m - 1 elements in the subarray
  ...
  *
  * 1. For the first part of the programming assignment, you should always use the first element of the array
  * as the pivot element.
  * 2. Compute the number of comparisons (as in Problem 1), always using the final element of the given array as
  * the pivot element. Again, be sure to implement the Partition subroutine exactly as it is described in the
  * video lectures.
  ...
  * [The primary motivation behind this rule is to do a little bit of extra work to get much better performance
  * on input arrays that are nearly sorted or reverse sorted.] In more detail, you should choose the pivot as follows.
  * Consider the first, middle, and final elements of the given array. (If the array has odd length it should be clear
  * what the "middle" element is; for an array with even length 2k, use the kth element as the "middle" element.
  * So for the array 4 5 6 7, the "middle" element is the second one ---- 5 and not 6!) Identify which of these
  * three elements is the median (i.e., the one whose value is in between the other two), and use this as your pivot.
  * As discussed in the first and second parts of this programming assignment, be sure to implement Partition exactly
  * as described in the video lectures (including exchanging the pivot element with the first element just before
  ...
  */
object Assignment2 {
  def quicksort(a: Array[Int], choosePivot: (Int, Int, Array[Int]) => Int): Int = {
    ...
    def quicksort(l: Int, r: Int): Int = {
      if (l >= r) 0
        ...
        val i = choosePivot(l, r, a)
        swap(l, i)
        val j = partition(l, r)
        val x = quicksort(l, j - 1)
        val y = quicksort(j + 1, r)
        /*
         * The height of a recursion tree is n times the factor raised to some power such that the RHS = 1, the
         * smallest subproblem size.
         * For a binary tree
         * n * (1/2)^h = 1
         * n = 2^h
         ...
         * h = log₂n
         *
         * c.f. http://www.algorist.com/algowiki/index.php/TADM2E_4.17
         * If the subproblems are unequally divided, say n/3 and 2n/3, then clearly, the larger part will involve
         * more recursive calls
         * n * (2/3)^h = 1
         * n = (3/2)^h
         * Taking log[base 3/2] on both sides
         * h = log[base 3/2](n)
         *   = log[base2](n) / log[base2](3/2)
         *   ~= 1.71log(n)
         */
        // size of the array passed to the partition method - 1 (the pivot is not compared to itself) + results from
        // the recursive calls
        r - l + x + y
      }
    }
