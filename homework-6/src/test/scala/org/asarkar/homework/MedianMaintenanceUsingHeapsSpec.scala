...
class MedianMaintenanceUsingHeapsSpec extends FlatSpec with TableDrivenPropertyChecks {
  val n = 10000

  "MedianMaintenanceUsingHeaps" should "compute medians of the given numbers" in {
    val data =
    // format: off
    ...
    forAll(data) { (numbers, sumOfMediansModulo10000) =>
      MedianMaintenanceUsingHeaps(numbers)
    ...
    ZipUtil.transformEntry(
      ...
      case Success(xs) =>
        ...
        ZipUtil.transformEntry(
          ...
          case Success(ys) => MedianMaintenanceUsingHeaps(xs).medians shouldBe ys
        }
    }
  }
  ...
  it should "compute medians of the numbers from the assignment file" in {
    ...
    ZipUtil.transformEntry(
      ...
      case Failure(e) => fail("Failed to read input file", e)
      case Success(xs) => MedianMaintenanceUsingHeaps(xs).medians.sum % n shouldBe 1213
    }
  }
}
